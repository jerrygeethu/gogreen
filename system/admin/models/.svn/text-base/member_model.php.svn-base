...
var $insert = array ("name"  => "",
   ...
   function check_member_data($fieldname,$value){
	...
	function memberedit($insert,$id)
	{
		
	$this->db->update('member', $insert, array('memberid' => $id));
	$this->db->last_query();
    return $this->db->insert_id(); 	
	...
	function plotadd($ins)
	{
		
	$this->db->insert('plots', $ins);
	$this->db->last_query();
    return $this->db->insert_id(); 	
	...
	function allactivities($d)
	{
		...
		function memberlist($id="",$limit,$perpage)
		{
